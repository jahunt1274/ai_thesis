...
USAGE_RESULTS_DIR = os.environ.get(
    "USAGE_RESULTS_DIR", str(Path(ANALYSIS_RESULTS_DIR) / "usage")
)
COMBINED_RESULTS_DIR = os.environ.get(
    "AI_THESIS_COMBINED_RESULTS_DIR", str(Path(ANALYSIS_RESULTS_DIR) / "combined")
)

# Define paths for course evaluation data
COURSE_EVAL_DIR = os.environ.get(
    "AI_THESIS_COURSE_EVAL_DIR", str(Path(DATA_DIR) / "course_evaluations")
)

# Define path for visualization output
VISUALIZATION_OUTPUT_DIR = os.environ.get(
    "AI_THESIS_VISUALIZATION_OUTPUT_DIR", str(Path(OUTPUT_DIR) / "visualizations")
)
...
USER_DATA_FILE = os.environ.get(
    "AI_THESIS_USER_DATA", str(Path(DATA_DIR) / "users.json")
)
IDEA_DATA_FILE = os.environ.get(
    "AI_THESIS_IDEA_DATA", str(Path(DATA_DIR) / "ideas.json")
)
STEP_DATA_FILE = os.environ.get(
    "AI_THESIS_STEP_DATA", str(Path(DATA_DIR) / "steps.json")
)
CATEGORIZED_IDEA_FILE = os.environ.get(
    "AI_THESIS_CATEGORIZED_IDEA_DATA",
    str(Path(DATA_DIR) / "categorized_ideas_latest.json"),
)
...
COURSE_EVAL_SCHEMA = os.environ.get(
    "AI_THESIS_COURSE_EVAL_SCHEMA",
    str(Path(SCHEMA_DIR) / "course_evaluation_schema.json"),
)
