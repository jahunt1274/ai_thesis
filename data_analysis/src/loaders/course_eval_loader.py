    ...
    def __init__(self, eval_dir: str = COURSE_EVAL_DIR):
        ...
        if os.path.exists(COURSE_EVAL_SCHEMA):
            try:
                ...
                logger.info(
                    f"Loaded course evaluation schema from {COURSE_EVAL_SCHEMA}"
                )
                    ...
                    for eval in eval_data:
                        ...
                        if self.schema:
                            try:
                                ...
                                logger.warning(
                                    f"Validation failed for {filename}: {str(e)}"
                                )
                                continue
    ...
    def _process_evaluation(
        ...
        try:
            ...
            overall_metrics = self._calculate_overall_metrics(
                eval_data.get("evaluation_metrics", [])
            )
            ...
            logger.error(
                f"Error processing evaluation for {eval_data.get('course_id', 'unknown')}: {str(e)}"
            )
            return None
