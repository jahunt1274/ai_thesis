...
class StatsUtils:
    ...
    def calculate_trend(values: List[float]) -> Dict[str, Any]:
        ...
        if not values or len(values) < 2:
            return {"direction": "unknown", "slope": None, "consistent": None}
    ...
    def calculate_correlation(
        ...
        if (
            ...
            return {"correlation": None, "direction": "unknown", "strength": "unknown"}
        ...
        covariance = (
            sum((x_values[i] - mean_x) * (y_values[i] - mean_y) for i in range(n)) / n
        )
    ...
    def distribution_buckets(
        values: List[float], bucket_size: float = 10.0
    ) -> Dict[float, int]:
        """
