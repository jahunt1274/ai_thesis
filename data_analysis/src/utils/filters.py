...
class DataFilter:
    ...
    def filter_by_course(
        ...
        for user in users:
            ...
            if any(
                str(enrollment).startswith(course_code) for enrollment in enrollments
            ):
                if user.get("email"):
    ...
    def filter_by_activity(
        ...
        for user in users:
            ...
            if (
                ideas_by_user.get(email, 0) >= min_ideas
                and steps_by_user.get(email, 0) >= min_steps
            ):
    ...
    def filter_by_time_period(
        ...
        filtered_users = [
            user for user in users if is_in_range(user.get("created_date"))
        ]

        # Filter ideas by creation date
        filtered_ideas = [
            idea for idea in ideas if is_in_range(idea.get("created_date"))
        ]
    ...
    def _apply_user_filter(
        ...
        filtered_idea_ids = [
            idea.get("id") for idea in filtered_ideas if idea.get("id")
        ]
