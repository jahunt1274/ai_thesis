...
class CourseEvaluationAnalyzer(BaseAnalyzer):
    ...
    def _analyze_semester_comparison(self) -> Dict[str, Any]:
        ...
        semester_sort_data = [
            (code, semester_data[code]["year"], semester_data[code].get("order", 0))
        ...
        comparison = {
            ...
            "display_names": [
                semester_data[code]["display_name"] for code in sorted_semesters
            ],
            "overall_avg": [
                semester_data[code]["overall_metrics"].get("overall_avg")
                for code in sorted_semesters
            ],
            "tool_versions": [
                semester_data[code].get("tool_version") for code in sorted_semesters
            ],
    ...
    def _analyze_tool_impact(self) -> Dict[str, Any]:
        ...
        for version, evals in version_data.items():
            avg_scores = [
                e["overall_metrics"].get("overall_avg")
                for e in evals
        ...
        for version, evals in version_data.items():
            ...
            spring_scores = [
                e["overall_metrics"].get("overall_avg")
                for e in spring_evals
                if e["overall_metrics"].get("overall_avg") is not None
            ]

            seasonal_impact[version] = {
                "fall_avg": (
                    sum(fall_scores) / len(fall_scores) if fall_scores else None
                ),
                "spring_avg": (
                    sum(spring_scores) / len(spring_scores) if spring_scores else None
                ),
    ...
    def _analyze_section_performance(self) -> Dict[str, Any]:
        ...
        for eval_data in self.evaluations:
            ...
            for section in eval_data.get("evaluation_metrics", []):
                ...
                valid_scores = [
                    q.get("avg") for q in questions if q.get("avg") is not None
                ]

                if valid_scores:
                    section_averages[section_name] = sum(valid_scores) / len(
                        valid_scores
                    )
            ...
            for eval_data in self.evaluations:
                ...
                for section in eval_data.get("evaluation_metrics", []):
                    for question in section.get("questions", []):
                        ...
                        if any(
                            possible_q.lower() in question_text.lower()
                            for possible_q in possible_questions
                        ):
    ...
    def _analyze_rating_trends(self) -> Dict[str, Any]:
        ...
        for i in range(1, len(timeline)):
            ...
            if (
                prev.get("overall_avg") is not None
                and curr.get("overall_avg") is not None
            ):
                change = curr["overall_avg"] - prev["overall_avg"]
                pct_change = (
                    change / prev["overall_avg"] * 100 if prev["overall_avg"] else None
                )
        ...
        for term in ["fall", "spring"]:
            ...
            if len(term_evals) >= 2:
                term_trend = StatsUtils.calculate_trend(
                    [e.get("overall_avg") for e in term_evals]
                )
                term_trends[term] = {
        ...
        return {"timeline": timeline, "changes": changes, "term_trends": term_trends}
    ...
    def _analyze_time_spent(self) -> Dict[str, Any]:
        ...
        sorted_evals = sorted(self.evaluations, key=self._semester_sort_key)
        ...
        for eval_data in sorted_evals:
            ...
            for section in eval_data.get("evaluation_metrics", []):
                ...
                if section_name == "Time Spent":
                    self.logger.info(
                        f"Found 'Time Spent' section for semester {semester_code}"
                    )
        ...
        time_data["total_time_timeline"] = [
            entry for entry in time_data["timeline"] if entry.get("is_total", False)
        ]
            ...
            time_data["changes"].append(
                {
                    ...
                    "change": change,
                    "percent_change": pct_change,
                    "tool_change": prev_data.get("tool_version")
                    != curr_data.get("tool_version"),
        ...
        if len(sorted_semesters) >= 2:
            total_times = [
                time_data["by_semester"][s].get("total_time", 0)
        ...
        self.logger.info(
            f"Time spent analysis found {len(time_data['timeline'])} data points"
        )
    ...
    def _analyze_overall_rating(self) -> Dict[str, Any]:
        ...
        for eval_data in sorted_evals:
            ...
            for section in eval_data.get("evaluation_metrics", []):
                for question in section.get("questions", []):
                    ...
                    if any(
                        overall_phrase in question_text
                        for overall_phrase in overall_questions
                    ):
                        avg_score = question.get("avg")
                        if avg_score is not None:
            ...
            if overall_scores:
                ...
                rating_data["by_semester"][semester_code]["num_questions"] = len(
                    overall_scores
                )
        ...
        for i in range(1, len(versions)):
            ...
            if (
                prev_version in rating_data["avg_by_tool_version"]
                and curr_version in rating_data["avg_by_tool_version"]
            ):

                prev_rating = rating_data["avg_by_tool_version"][prev_version][
                    "avg_rating"
                ]
                curr_rating = rating_data["avg_by_tool_version"][curr_version][
                    "avg_rating"
                ]
                ...
                version_changes.append(
                    {
                        ...
                        "percent_change": (
                            (curr_rating - prev_rating) / prev_rating * 100
                            if prev_rating
                            else None
                        ),
                    }
        ...
        sorted_timeline = sorted(
            rating_data["timeline"], key=lambda x: x["semester_code"]
        )
        ...
        for i in range(1, len(sorted_timeline)):
            ...
            pct_change = (
                change / prev["avg_rating"] * 100 if prev["avg_rating"] else None
            )
        ...
        rating_data["correlation_with_time"] = {
            ...
            "correlation": (
                StatsUtils.calculate_correlation(
    ...
    def _compare_terms(
        ...
        return {
            ...
            "percent_difference": (
                (seasonal_diff / spring_avg * 100)
                if spring_avg and seasonal_diff is not None
                else None
            ),
        }
